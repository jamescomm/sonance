				...
				parseFloat(remainingVolume)
					...
					else
					{
						...
						Async.waterfall([
							...
							function(success,cb)
							{
								currentBid.amount.value = new BigNumber(currentBid.amount.value);
								let amount = currentBid.amount.value.minus(Ask.amount.value);
								currentBid.volume.value = new BigNumber(currentBid.volume.value);
								let volume = currentBid.volume.value.minus(Ask.volume.value);
								parseFloat(amount)
								parseFloat(volume)
				...
				Async.forEachLimit(successed,1,(currentAsk,next)=>{
					...
					else
					{
						...
						Async.waterfall([
							...
							function(success,cb)
							{
									currentAsk.amount.value = new BigNumber(currentAsk.amount.value)
									let amount =  currentAsk.amount.value.minus(Bid.amount.value);
									currentAsk.volume.value = new BigNumber(currentAsk.volume.value)
									let volume = currentAsk.volume.value.minus(Bid.volume.value);
									parseFloat(amount)
									parseFloat(volume)
