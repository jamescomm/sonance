...
let exchangeAsk = (Ask,User,userCurrency,callback)=>{
	...
	else
	{
		...
		.then((success)=>console.log("User Ask balance updated initialy : ",success))
		.catch((unsuccess)=>callback(unsuccess))
		Order.find({'volume.currency':Ask.volume.currency,'amount.currency':Ask.amount.currency,type:'ASK',status:false,rate:Ask.rate}).sort({createdAt:-1})
		.then((successed)=>{
			if(successed.length)
			{ let counter = 0;
				...
				Async.forEachLimit(successed,1,(currentBid,next)=>{
					...
					if(remainingVolume == 0)
					{
						Async.waterfall([
							...
							function(success,cb)
							{
							Order.update({_id:currentBid._id},{'amount.value':0,'volume.value':0,status:true},{new:true})
							.then((success)=>cb(null,success))
							.catch((unsuccess)=>cb(unsuccess))
							}
							],
					...
					else if(remainingVolume>0)
					{
						Async.waterfall([
							...
							function(success,cb)
							{
							Order.update({_id:currentBid._id},{'amount.value':0,'volume.amount':0,status:true},{new:true})
							.then((success)=>cb(null,success))
							.catch((unsuccess)=>cb(unsuccess))
							}
							],
							function(err,success)
							{
								...
								{
								let amount = Ask.amount.value - currentBid.amount.value;
								let volume = Ask.volume.value - currentBid.volume.value
								var market = Ask.amount.currency+'/'+Ask.volume.currency;
								Order.create({status:true,'amount.currency':Ask.amount.currency,'amount.value':amount,rate:Ask.rate,'volume.currency':Ask.volume.currency,'volume.value':volume,type:'Ask',userId:Ask.user_id,createdAt:new Date(),processedAt:new Date(),market:market,total_amount:amountOfInitialAsk,total_volume:volumeOfInitialAsk})
								.then((success)=>cb(null,success))
								.catch((unsuccess)=>cb(unsuccess))
								}
								}
					...
					else
					{
						Async.waterfall([
							...
							function(success,cb)
							{
								...
								Order.update({_id:currentBid._id},{$inc:{'amount.value':amount,'volume.value':volume}},{new:true})
								.then((success)=>cb(null,success))
								.catch((unsuccess)=>cb(unsuccess))
							}
							],
					...
					else if(remainingVolume>0)
					{
						Async.waterfall([
							...
							function(err,success)
							{
								...
								{
								let amount = Bid.amount.value - currentAsk.amount.value;
								let volume = Bid.volume.value - currentAsk.volume.value;
								var market = Bid.amount.currency+'/'+Bid.volume.currency;
								Order.create({status:true,'amount.currency':Bid.amount.currency,'amount.value':amount,rate:Bid.rate,'volume.currency':Bid.volume.currency,'volume.value':volume,type:'BID',userId:Bid.user_id,createdAt:new Date(),processedAt:new Date(),market:market,total_amount:amountOfInitialBid,total_volume:volumeOfInitialBid})
								.then((success)=>cb(null,success))
								.catch((unsuccess)=>cb(unsuccess))
								}
								}
