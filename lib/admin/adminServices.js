...
module.exports = {
'addQuestionAns':(req,res)=>{
if(req.body.question && req.body.ans)
  QuesAns.create({Ques:req.body.question,Ans:req.body.ans})
  ...
  .catch((unsuccess)=>{return resHndlr.apiResponder(req, res,'Something went wrong' , 500)})
    ...
    return resHndlr.apiResponder(req, res,'Please provide the Q&A.' , 500)
  },
'getQuestionAns':(req,res)=>{
QuesAns.find()
  .then((success)=>{return resHndlr.apiResponder(req, res,'Success' , 200,success)})
  .catch((unsuccess)=>{return resHndlr.apiResponder(req, res,'Something went wrong' , 500)})
  },
  'removeQuestionAns':(req,res)=>{
    if(req.body._id)
      QuesAns.findOneAndRemove({_id:req.body._id})
  .then((success)=>{return resHndlr.apiResponder(req, res,'Success' , 200,success)})
  .catch((unsuccess)=>{return resHndlr.apiResponder(req, res,'Something went wrong' , 500)})
  else
    return resHndlr.apiResponder(req, res,'Please provide the Q&A.' , 500)
  },
'addApiContent':(req,res)=>{
if(req.body.title && req.body.body)
  ApiContent.create({title:req.body.title,body:req.body.body})
  .then((success)=>{return resHndlr.apiResponder(req, res,'Success' , 200,success)})
  .catch((unsuccess)=>{return resHndlr.apiResponder(req, res,'Something went wrong' , 500)})
  else
    return resHndlr.apiResponder(req, res,'Please provide the Q&A.' , 500)
  },
'getApiContent':(req,res)=>{
ApiContent.find()
  .then((success)=>{return resHndlr.apiResponder(req, res,'Success' , 200,success)})
  .catch((unsuccess)=>{return resHndlr.apiResponder(req, res,'Something went wrong' , 500)})
  },
  'removeApiContent':(req,res)=>{
    if(req.body._id)
      ApiContent.findOneAndRemove({_id:req.body._id})
  .then((success)=>{return resHndlr.apiResponder(req, res,'Success' , 200,success)})
  .catch((unsuccess)=>{return resHndlr.apiResponder(req, res,'Something went wrong' , 500)})
  else
    return resHndlr.apiResponder(req, res,'Please provide the Q&A.' , 500)
...
.then((result)=>{ 
   if(result)
  if(result.ip.indexOf(req.query.ip)<0)
  RuleBook.update({},{$push:{ip:req.query.ip}},{new:true})
  .then((success)=>{return resHndlr.apiResponder(req, res,'Success' , 200,success)})
  .catch((unsuccess)=>{return resHndlr.apiResponder(req, res,'Something went wrong' , 500)})
  else
    return resHndlr.apiResponder(req, res,'You have already performed this action.' , 500)
  else
    return resHndlr.apiResponder(req, res,'You have successfully blocked this ip.' , 500)
})
.catch((unsuccess)=>{return resHndlr.apiResponder(req, res,'Something went wrong' , 500)})
else
return resHndlr.apiResponder(req, res,'Please provide the ip.' , 500)
  },
    'unblockIp':(req,res)=>{
    req.query.ip
    if(req.query.ip)
    RuleBook.update({},{$pull:{ip:req.query.ip}},{new:true})
  .then((success)=>{return resHndlr.apiResponder(req, res,'Success' , 200,success)})
  .catch((unsuccess)=>{return resHndlr.apiResponder(req, res,'Something went wrong' , 500)})
  else
    return resHndlr.apiResponder(req, res,'Please provide the ip.' , 500)
...
'subadmin':(req,res)=>{
	...
	{
		User.create({email:req.body.emailId,role:'subadmin','state.status':'active'})
		.then((success)=>{
			 var text = '';
