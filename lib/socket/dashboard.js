...
.then((successed)=>{console.log("successed: ",successed.currencyData.length)
	Async.forEachLimit(successed.currencyData,1,(Currency,next)=>{
		...
		{
			console.log("11111",Currency.currency)
			var tsYesterday = new Date(new Date().getTime() - (144 * 60 * 60 * 1000));
			...
			console.log("in elseeeeee")
			let i = 0;
			console.log("successed.currencyData.length:",successed.currencyData.length)
			for (i; i < successed.currencyData.length; i++) {
				data = successed.currencyData[i]
				console.log("here ", data.market);
