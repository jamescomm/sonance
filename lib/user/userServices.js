...
function sendMessage(_id, number, message, callback) {
    client.messages.create({
        ...
        if (err) resHndlr.apiResponder(req, res, Constants.MESSAGES.RequiredField, 400)
        else {
            User.findOneAndUpdate({
                ...
                new: true
            }).then((success) => callback(null, 'OTP is send on your phone please verify.')).catch((unsuccess) => callback(Constants.MESSAGES.SomeThingWrong))
        }
    })
}
//*****************************************APIS***************************************************
...
bcrypt.genSalt(10, function(err, salt) {
    if(!req.body.email || !req.body.password || !req.body.name || !req.body.lastName) //user_id/page/sort/status/type
        ...
        console.log("userData:",userData)
         User.create(userData, function (err, user) {
            if (err) {
              ...
              globalFunction.sendMail(req.body.email,'subject','text',(err,result)=>
              {
                if(err)
                {
                    console.log("err: ",err)
                    ...
                    resHndlr.apiResponder(req, res, Constants.MESSAGES.evrifyMail, 200)
              })
            }
          });
    ...
    'login': (req, res) => {
        ...
        var userCriteria = {email: req.body.email}
        User.findOne(userCriteria, function(err, record) {
            if (err) return res.json(err)
            if (record) {
                bcrypt.compare(req.body.Password, record.password, (err, result) => {
                    if (result) {
                        var token = jwt.sign({id: record._id, role: record.role}, "exchange@#0131", {expiresIn: 86400});
                        User.findOneAndUpdate({userId: result._id},{$push:{'loginDetails.ip':req.body.ip,'loginDetails.location':req.body.location}},{new:true}, function(err, result) {
                            if (err) return res.json(err);
                            else {
                                record.token = token
                                return resHndlr.apiResponder(req, res, Constants.MESSAGES.PasswordMismatch, 200, record)
                            }
                        });
                    } else return resHndlr.apiResponder(req, res, Constants.MESSAGES.PasswordMismatch, 400)
                })
            } else return resHndlr.apiResponder(req, res, Constants.MESSAGES.EmailNotExsist, 400)
        })
    },
    ...
    'sendOtp': (req, res) => {
        if (!req.body._id || !req.body.number) return resHndlr.apiResponder(req, res, Constants.MESSAGES.RequiredField, 400)
        else {
    ...
    'verifyOtp': (req, res) => {
        if (!req.body._id || !req.body.otp) return resHndlr.apiResponder(req, res, Constants.MESSAGES.RequiredField, 400)
            ...
            if (success && success.otp == req.body.otp) resHndlr.apiResponder(req, res, 'OTP is virified successfully.', 200)
    ...
    'userList':(req,res)=>{
    	User.find().select('name middleName lastName state email createdAt role loginDetails verifyPhone isTwoFactor isKYC phone').exec()
