...
'getPrivateKey':(req,res)=>{
	Currencies.find({'currencies.currency':'BTC'},{'currencies.$':1}).select('userId currencies.address').lean().exec()
	.then((success)=>{
		...
		{
			console.log(address.currencies[0].address)
			client.cmd('dumpprivkey',address.currencies[0].address,function(error,successed){
				if(successed)
				{
