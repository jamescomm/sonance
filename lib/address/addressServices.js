...
'getPrivateKey':(req,res)=>{
	...
	.then((success)=>{
		...
		.then((credential)=>{
			...
			index = Math.ceil(req.query.skip/1000);
			if(req.query.currency != 'ETH'){
			console.log("11111",credential,index,credential[index-1]);
			if(credential[index-1])
			var client = new bitcoin.Client({
			  host: credential[index-1].host,
		...
		if(req.query.currency == 'BCH')
		{
			// client.cmd('walletlock',function(Err,succ)
			// {
			// 	if(Err)
			// 		console.log(Err)
			// 	else
			// 		console.log(succ)
			// })
			client.cmd('walletpassphrase',req.query.phrase,60,function(error,successed){
				console.log("pass::: ",error,successed)
				if(error)
					return resHndlr.apiResponder(req, res, error.message, 400)
				else
				{
			...
			client.cmd('dumpprivkey',found.address,function(error,successed){
				if(successed)
				{
					...
					if(address.identity)
						found.identity = address.identity
				found.privateKey = successed;
				data.push(found);
				counter++;
		...
		else
		{
			...
			client.cmd('dumpprivkey',found.address,function(error,successed){
				...
				return resHndlr.apiResponder(req, res, error.message, 401)
			})
		}
			}
			else
				return resHndlr.apiResponder(req, res,'Ethereum is under process', 400)
		  })
		})
	}
	else
		return resHndlr.apiResponder(req,res,'No address for BCH.',500)
	})
	.catch((unsuccess)=>{
