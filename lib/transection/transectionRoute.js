...
trnsRoute.route("/getWithdrawReq")
    .get([/*middleware.authenticate.autntctTkn*/], function (req, res) {
         // if(req.user.id == req.body.user_id)
        transectionServices.getWithdrawReq(req,res);
        // else
...
trnsRoute.route("/sendBalanceApproved")
    .post([/*middleware.authenticate.autntctTkn*/], function (req, res) {
        // if(req.user.id == req.body.user_id)
        transectionServices.sendBalanceApproved(req,res);
        // else
    //     resHndlr.apiResponder(req, res, "Unathorized access", 403)
    });
...
trnsRoute.route("/getUserWithdrawReq")
    .post([/*middleware.authenticate.autntctTkn*/], function (req, res) {
          // let { address } = req;
        transectionServices.getUserWithdrawReq(req,res);
    });
