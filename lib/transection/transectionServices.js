                ...
                globalFunction.isAddressExsist(response._id, 'XRP', function(err, result) {
                    if (err) return resHndlr.apiResponder(req, res, err, 500, err)
                        ...
                        const payment = {
                            ...
                            destination: {
                                address: 'rhTK9372JsqJ9baT1E7TXgTbkWV9RSu9xZ',amount: {value: req.body.amount,currency: 'XRP'}
                                // ,
                        ...
                        api.connect().then(() => {
                            ...
                            return api.preparePayment(req.body.address, payment).then(prepared => {
                                ...
                                const {signedTransaction} = api.sign(prepared.txJSON, 'sn2jczubzBakX1mXCDRWGrAAYnSMB');
...
'getXrpTransactions':(req,res)=>{
    const address = 'rwqasLmhJPpRYExYtiEe2JJ5BS3wXExWPA';
     api.connect().then(() => {
return api.getOrders(address)
.then(orders =>
  {
    return resHndlr.apiResponder(req, res, "Result",200,orders)
  })
.catch(unsuccess=>{
