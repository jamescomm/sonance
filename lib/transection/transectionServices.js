...
var accounts = new Accounts('http://192.64.116.199:8545');
var personal = new Personal('http://192.64.116.199:8545');
var web3 = new Web3(new Web3.providers.HttpProvider('http://192.64.116.199:8545'));
    ...
    if(fromAddress){
          ...
          if(data){
            ...
            return element.currency == currencyValue.currency;
             ...
             if(currencyDetailsInDb)
            {
              etherUpdate = BigNumber(data).minus(currencyDetailsInDb.ether);
              if(etherUpdate>0)
              {
                let updatedBalance = BigNumber(currencyDetailsInDb.balance).plus(etherUpdate);
                let ETHER = data
                globalFunction.updateEther(req.body.user_id, currencyValue.currency, updatedBalance,data, (err, result) => {
                    if (err)
                        return resHndlr.apiResponder(req, res, "Something went wrong", 500, err);
                    else
                        return resHndlr.apiResponder(req, res, "Successfully update your balance.", 200, result);
...
Credential.findOne({})
    ...
    // if(currencyValue.currency=='BCH')
    client.cmd('walletpassphrase',req.body.phrase,60,function(error,successed){
            client.cmd('sendfrom', currencyValue.COMPANYACCOUNT, req.body.address, parseFloat(sendAmount), 1,
                             req.body.address, req.body.address, (err, TransactionDetails, resHeaders) => {
                                if (err)
                                    ...
                                    client.cmd('walletlock',function(errr,succc)
                                        {
                                            if(errr)
    ...
    web3.personal.signAndSendTransaction({'to': req.body.address,
     'from': currencyValue.COMPANYACCOUNT ,'value': sendAmount}, req.body._id,(err, TransactionDetails)=>{
                                if (err)
                                    ...
                                    let updatedBalance = parseFloat(BigNumber(result.balance).minus(req.body.balance));
                                    let updatedEther = parseFloat(BigNumber(result.ether).minus(req.body.balance));
                                    globalFunction.updateEther(req.body.user_id, currencyValue.currency, updatedBalance,updatedEther, (err, result) => {
                                        if (err)
                                            return resHndlr.apiResponder(req, res, "Something went wrong", 500, err);
                                        else
                                            return resHndlr.apiResponder(req, res, "Successfully completed your transection.", 200, result);
                          ...
                          let sendAmount = parseFloat(BigNumber(req.body.balance).multipliedBy(currencyValue.transectionCharge));
                           ...
                           let updatedBalance = parseFloat(BigNumber(result.balance).minus(req.body.balance));
                                    globalFunction.updateBalanceInDb(req.body.user_id, currencyValue.currency, updatedBalance, (err, result) => {
                                        ...
                                        {
                                            Transaction.create({userId:req.body.user_id,amount:req.body.balance,currency:req.body.currency,identity:req.body.identity,address:req.body.address})
                                            .then((success)=>{
                                                return resHndlr.apiResponder(req, res, "Your request for withdraw is Successfully submitted, amount will be verified and proceed in one hour.", 200)
                                            })
                                            .catch((unsuccess)=>{
                                                return resHndlr.apiResponder(req, res, "Something went wrong", 500, v) 
                                            })
                                        }
                                        // transaction(req,res,req.body.user_id,TransactionDetails,req.body.balance,true);
                                    })
...
'getWithdrawReq':(req,res)=>{
    Transaction.find({status:req.query.status})
    .then((success)=>{
        return resHndlr.apiResponder(req, res, "Result", 200, success)
    })
    .catch((err)=>{
        return resHndlr.apiResponder(req, res, "Something went wrong", 500, err)
    })
},
'getUserWithdrawReq':(req,res)=>{
    Transaction.find({userId:req.body.user_id,currency:req.body.currency})
    .then((success)=>{
        return resHndlr.apiResponder(req, res, "Result", 200, success)
    })
    .catch((err)=>{
